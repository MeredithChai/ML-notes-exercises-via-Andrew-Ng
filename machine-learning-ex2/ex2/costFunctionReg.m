...
% 12-05 Me:
    ...
    theta_tmp = [0; theta(2:end,1)];    % theta;
    J = -1/m * sum(y.*log(h_x) + (1-y).*log(1-h_x)) + lambda/(2*m) * sum(theta_tmp.^2);
    % or 
    % J = -1/m * (y'*log(h_x) + (1-y)'*log(1-h_x)) + lambda/(2*m) * sum(theta_tmp.^2);
    grad = -1/m * X' * (y-h_x) + lambda/m * theta_tmp;
